<x-original-layout.format title="プロフィールの編集">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col items-center m-5">
            ...
            <form method="POST" action="{{ route('user.userProfile.update', ['id' => auth()->user()->id]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="flex justify-center mb-12">
                    <a href="{{ url()->previous() }}" class="mx-4 py-2 w-32 border border-gray-500 rounded bg-gray-500 text-white text-center hover:border-gray-600 hover:bg-gray-600">戻る</a>
                    <button type="submit" class="mx-4 py-2 w-32 border border-emerald-700 rounded bg-emerald-700 text-white hover:border-emerald-800 hover:bg-emerald-800">変更する</button>
                </div>
            </form>
            <div class="flex justify-center mb-10">
                <a href="{{ route('user.userProfile.deleteConfirmation') }}" class="py-2 w-44 border border-red-300 rounded bg-red-300 text-black text-center hover:border-red-500 hover:bg-red-500 hover:text-white">アカウントを削除</a>
            </div>
        </div>
    </div>
